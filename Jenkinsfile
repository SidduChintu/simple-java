  ...
  stages {
   stage ('Initialize') {
    ...
    stage('Push Artifact to S3') {
      steps {
        sh 'aws s3 cp webapp/target/webapp.war s3://sid-bucket-jenkins'
      }
    }
    
    stage('Deploy to tomcat') {
      steps {
        sh 'sudo scp -i $tomcat_pem -o "StrictHostKeyChecking=no" webapp/target/webapp.war ubuntu@65.1.109.181:/opt/tomcat/webapps'
      }
    }
